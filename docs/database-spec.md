...
- `execution_costs` と `execution_costs.commission` は optional とする。commission のフィールド不在は legacy order、broker 未対応、約定情報の欠落などで値が unknown であることを表し、`0` とは区別する。注文更新 API では field 不在を `null` として公開する（[注文更新 API OpenAPI](./openapi/order-updates.openapi.yaml)、[API 利用仕様](./api-spec.md#order-updates-api)）。
